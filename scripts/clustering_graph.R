...
D0.D15 <- read.csv("data/diff_expression/D0_D15.csv_annotated.csv", stringsAsFactors = FALSE)
D15.D150 <- read.csv("data/diff_expression/D15_D150.csv_annotated.csv", stringsAsFactors = FALSE)
...
table.logFC$comb.pvalue.discrete <- table.logFC$comb.pvalue < 10^-4

table.logFC$gene.name <- ifelse (table.logFC$gene.name == "", D0.D15$Xenopus.Associated.Gene.Name[ match(table.logFC$contig, table = D0.D15$contig) ], table.logFC$gene.name)
