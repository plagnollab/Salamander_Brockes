...
refseq_annotation = pickle.load(open("blast_dict.p"))
...
""" contig dictionary is necessary for gene level
for row in csv.reader(open("raw_count/D0_D15.csv")):
    
    
    #print row[-1]
    
    
    gene_ = row[-1].split("_")[0]
    
    if gene_ not in contig_2_gene_dict:
        
        contig_2_gene_dict[gene_] = []
        
    contig_2_gene_dict[gene_].append(row[-1])
    
    #raw_input()

pickle.dump(contig_2_gene_dict,open("contig_2_gene_dict","w"))
#"""




contig_2_gene_dict = pickle.load(open("contig_2_gene_dict"))
...
for file_ in os.listdir(blst_directory):
    ...
    for row in read_csv:
        ...
        if row[0].split("_")[-1] == "seq1":
            ...
            if idz == "human":
                blast_master_dict[idz][row[0].split("_s")[0]] = [row[1].split("|")[3],row[2],row[10]]
                
                #print row[1].split("|")[3],row[2],row[10]
                #raw_input()                
    ...
    if file_.endswith(".csv") and file_.find("annot") == -1:
        ...
        for row in read_csv:
            ...
            if contig_index == None:
                ...
                id_GENE = row[contig_index]

            if id_GENE not in contig_2_gene_dict:
                
                print "Not in table",id_GENE
                continue
            
            contig_id_list = contig_2_gene_dict[id_GENE]
            
            
            #print contig_id_list
            
            #raw_input("")
            highest_percent_match = 0
            min_e = 1            
            best_id = ""
            
            #if len(contig_id_list) <= 2:
            #    continue
            
            
            
            for id_c in contig_id_list:
                                       
                if id_c not in blast_master_dict["human"]:
                    continue
                
                
                [id_x,pi_x,eval_x] = blast_master_dict["human"][id_c]
                
                
                if float(pi_x) > highest_percent_match and float(eval_x) <= min_e:
                    
                    highest_percent_match = pi_x  
                    min_e = eval_x 
                    best_id = id_c
                    
                #print [id_c,id_x,pi_x,eval_x]
